<script lang="ts">
    import { useHail, useReadHail } from "../../setup/svelte/ahoi";
...
<div class="demo">
    ...
    <button id="bump0" on:click={() => ($first = ($first ?? 0) + 1)}>+1 on items[0]</button>
    <button id="bump1" on:click={() => ($second = ($second ?? 0) + 1)}>+1 on items[1]</button>
</div>
