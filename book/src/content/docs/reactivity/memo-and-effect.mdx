...
import MemoIsland from '../../../../examples/memo/solid/island.tsx';
...
That last claim is the kind you normally have to take on faith — nothing
happening looks exactly like nothing happening.

So this demo counts it. The `label` memo increments a counter every time its
body actually runs.

<Example name="memo" jsOnly>
  <MemoIsland client:only="@astrojs/solid-js" slot="demo" />
</Example>

Press `+2` a few times: `count` climbs, `parity` and `label` sit still, and the
run counter **does not move**. Press `+1` and it ticks up by one.

That is the whole point of the `PartialEq` bound — and note the counter is a
hail like any other, so it arrives in the same dispatch as the value it
explains.
